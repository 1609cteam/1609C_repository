...
class Shop(unittest.TestCase):  # 继承
    ...
    def test_sptj(self):
        ...
        # log.debug('正在测试添加...')
        time.sleep(1)
        name=self.drv.find_element_by_name('name').send_keys('西红柿01')
        time.sleep(1)
        price=self.drv.find_element_by_name('price').send_keys(18)
        time.sleep(1)
        # jj=self.drv.find_element_by_name('jj').send_keys('补充维生素')
        # time.sleep(1)
        ...
        self.assertEqual(code, 400, msg='添加失败!!!')
        time.sleep(6)


    # 修改商品
    def test_spxg(self):
        self.drv.get('http://127.0.0.1:8000/sps/5')
        print('正在测试修改商品...')
        # log
        # log01.debug('正在测试修改...')
        time.sleep(1)
